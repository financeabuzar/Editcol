  ...
  return (
    <div className="min-h-screen grid md:grid-cols-2 fade-in">
      <div className="hidden md:flex bg-ink relative overflow-hidden p-12 items-end">
        ...
        <div className="absolute -bottom-32 -right-32 h-96 w-96 rounded-full" style={{ background: "radial-gradient(circle, rgba(223,255,0,0.25), transparent 70%)" }} />
        <div className="relative max-w-sm text-white">
          <Logo size="lg" as="div" />
          <p className="mt-8 font-heading text-3xl leading-tight">Welcome to EditCol.</p>
          <p className="mt-4 text-sm text-gray-400">Profiles, projects, messages, and reviews in one trusted workspace.</p>
        </div>
      </div>
        ...
        <form onSubmit={submit} className="w-full max-w-sm space-y-4" data-testid="login-form">
          ...
          {step === "password" ? (
            <>
              ...
              <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2">
                  <input data-testid="login-remember" type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="h-4 w-4 accent-[#39FF14]" />
                  <span className="text-gray-700">Remember me</span>
                </label>
                <Link to="/forgot-password" data-testid="login-forgot" className="font-semibold text-gray-900 hover:underline">Forgot?</Link>
              </div>

              {err && <p className="text-sm text-red-600" data-testid="login-error">{err}</p>}

              <button disabled={busy} data-testid="login-submit" className="btn-primary inline-flex w-full items-center justify-center gap-2 disabled:opacity-50">
            ...
            <div className="space-y-4">
              ...
              {otpDev?.login_otp && (
                <div className="card border-dashed border-gray-300 bg-gray-50 p-3 font-mono text-xs text-gray-700">
