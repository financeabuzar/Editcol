...
import { Menu, X, MessageCircle, LayoutDashboard, LogOut, Shield, Search } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-ink/95 backdrop-blur supports-[backdrop-filter]:bg-ink/80" style={{ background: "rgba(10,10,10,0.92)", backdropFilter: "blur(10px)" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between text-white">
        ...
        <button onClick={() => setOpen(!open)} data-testid="nav-menu-toggle" className="md:hidden text-white p-2">
          {open ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>

      {open && (
        <div className="md:hidden bg-ink text-white px-6 pb-5 space-y-3 border-t border-white/10">
          {links.map((l) => (
